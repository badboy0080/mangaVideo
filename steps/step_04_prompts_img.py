...
1. **资产清单（人物 / 场景）**：调用 DeepSeek，**主要依据** Step 1 纲要中的「【人物与场景清单】」
...
def _legacy_assets_to_catalog(assets: dict | None) -> dict[str, Any]:
    ...
    assets = assets or {}
    out = _empty_catalog()
    for key, tag in [("characters", "character"), ("scenes", "scene")]:
...
def _catalog_has_items(catalog: dict[str, Any]) -> bool:
    return bool(catalog.get("characters") or catalog.get("scenes"))
...
def _attach_script_refs(catalog: dict[str, Any], script: str) -> dict[str, Any]:
    ...
    for i, shot in enumerate(shots[1:], 1):
        for ref in set(_AT_TAG_RE.findall(shot)):
            ...
            if not tag:
                continue
            bucket = catalog.get({"character": "characters", "scene": "scenes"}[tag])
...
def _deepseek_extract_asset_catalog(
    ...
    """
    用 DeepSeek 从 Step 1 清单生成人物/场景；道具不再作为单独资产生成。
    返回 (catalog, 来源说明)。
    """
    ...
    print("  [DeepSeek] 根据 Step 1 清单生成人物 / 场景列表…")
    raw = deepseek_chat(
    ...
    catalog["props"] = []
    catalog["total_props"] = 0
    print(
        f"  资产清单：{catalog['total_chars']} 角色 / "
        f"{catalog['total_scenes']} 场景"
    )
    ...
    *,
    ...
    if inv_block.strip():
        user_body = (
            ...
            f"当前资产名称：{name}\n"
            f"资产类型：{tag}\n"
            f"资产描述：{auxiliary[:1400]}\n\n"
            "请按 SYSTEM_PROMPT 输出一段图片生成 prompt。"
        )
    else:
        user_body = (
            "未拿到完整项目创意正文，请保守生成，不要发明新设定。\n\n"
            f"资产名称：{name}\n描述：{auxiliary}\n类型（character/scene）：{tag}\n\n"
            "请按 SYSTEM_PROMPT 输出一段图片生成 prompt。"
        )
...
def run(assets: dict | None, script: str, out_dir: str, research: dict | None = None) -> dict:
    ...
    会先调用 DeepSeek 生成/更新 `assets.json`（人物、场景；props 保持空数组兼容旧结构）。
    ...
    for shot_num, shot_content in shot_blocks:
        ...
        shots_out[sk] = {
            ...
            "reference_tag_priority": ["scene", "character"],
            "kind": "shot",
            "tag": "shot",
        }
