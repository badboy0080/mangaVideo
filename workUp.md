...
## 2026-05-28 - 修复 .env.example 乱码并去掉密钥

### 本轮用户想做什么
你发现 `.env.example` 里有很多乱码，希望根据本地 `.env` 复制一份模板，但把真实密钥去掉，再放回 `.env.example`。

### 实际完成了什么
- 没有打印 `.env` 里的真实密钥，只读取了变量名。
- 将 `.env.example` 改成干净中文模板。
- 保留 3 个变量：`DEEPSEEK_API_KEY`、`ARK_API_KEY`、`VOLCENGINE_API_KEY`。
- 所有变量值都已清空，没有真实密钥。

### 验证
- 检查 `.env.example` 中没有 `=实际值` 形式的密钥内容。
- `git diff --check -- .env.example` 通过；只有 Windows 换行提示，不影响。

### 提问是否合理
合理。`.env.example` 是给别人配置环境用的，必须清楚、不能有乱码，也不能包含真实密钥。这个问题在提交前处理很重要。
